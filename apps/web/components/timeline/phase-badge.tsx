...
const STATUS_TONE: Record<TimelinePhase["status"], string> = {
  ...
  active: "border-[var(--color-accent-300)] bg-[var(--color-accent-100)] text-[var(--color-accent-800)]",
...
export function PhaseBadge({ phase, span }: PhaseBadgeProps) {
  ...
  return (
    <div
      className={`absolute rounded-md border px-1.5 py-0.5 font-mono text-[10px] uppercase tracking-wider ${STATUS_TONE[phase.status]}`}
