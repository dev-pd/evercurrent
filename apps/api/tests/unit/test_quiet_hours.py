"""Pure unit tests for notify.quiet_hours.

No DB, no Slack, no mocking. The functions are deterministic; we feed
them aware UTC datetimes + a `ZoneInfo` + start/end `time`s and assert
on the boolean / next-open instant.
"""
...
def test_respects_user_timezone_la_vs_kolkata() -> None:
    """Same UTC instant should produce different quiet results per tz.

    `2026-06-07 04:00 UTC` is
      - 21:00 the prior day in Los Angeles (active under 22:00-07:00)
      - 09:30 in Kolkata (active under 22:00-07:00)
    `2026-06-07 06:00 UTC` is
      - 23:00 in Los Angeles (quiet)
      - 11:30 in Kolkata (active)
    """
    ...
    assert is_within_quiet(
        instant_a, tz=kolkata, quiet_start=start, quiet_end=end,
    ) is False
    ...
    assert is_within_quiet(
        instant_b, tz=kolkata, quiet_start=start, quiet_end=end,
    ) is False
