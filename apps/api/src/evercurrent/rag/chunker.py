...
Strategy: parse headers; if a section fits within max_tokens emit it as
one chunk with its section_path. If it exceeds, recursively split via
the separators list with overlap. Approximates Voyage's tokenizer with
len(text) // 4.
"""
...
_TOKEN_RATIO = 4  # chars per token approximation
...
def _recursive_split(text: str, max_tokens: int, overlap_tokens: int) -> list[str]:
    ...
    for sep in _SEPARATORS:
        if sep == "":
            # Hard fallback: slice by max_chars.
        ...
        if sep in text:
            ...
            for piece in pieces:
                ...
                if current:
                    ...
                    # Carry a tail of the just-emitted chunk forward as overlap.
                    tail = current[-overlap_chars:] if overlap_chars else ""
                    current = (tail + sep + piece) if tail else piece
                else:
                    # A single piece is bigger than max — recurse on the next sep.
...
def _split_into_sections(text: str) -> list[tuple[str, str]]:
    """Yield (section_path, body) tuples in document order.

    \"Section path\" is constructed from the breadcrumb of seen headers
    (e.g. \"# Intro > ## Goals\").
    """
    ...
    # Prefix before the first header counts as the document preamble.
    ...
    for i, m in enumerate(matches):
        ...
        # Body runs from end of this header to start of next (or EOF).
        body_start = m.end()
...
def chunk_markdown(
    ...
    *,
    ...
    """Split markdown into chunks preserving section paths."""
    chunks: list[Chunk] = []
    chunk_index = 0
